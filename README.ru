Руководство по использованию программы "Downloader for X" версии 1.25
...
10. Интересные возможности
	10.1 FTP-поиск
	10.2 Планировщик
	10.3 Пароли по умолчанию
11. Заключение.
---------------------------------------------------------------------
...
лога, FTP-поиск. Для перемещения выбранных закачек вверх или вниз можно
...
2.6 Область FTP-поиска

Эта область становится доступной когда у FTP-поиска есть хотя бы одно
задание. Все задания маркируются точно также как закачки в очереди. После
того как поиск завершился вы можете нажать правой кнопкой мыши на
задании и выбрать наиболее подходящий сайт-зеркало. Все найденные сайты
сортируются по времени доступа.
...
Этот путь может содержать специальные форматеры, подстроки указывающие
программе что вместо них надо подставить его значение.  Форматеры и
их значения:
%[D] - текущий день месяца, будет занимать два знака
%[M] - текущий месяц, два знака, число от 01 до 12
%[Y] - текущий год
%[h] - текущей час, тоже два знака, от 00 до 23
%[m] - минуты текущего часа, 00-59
%[s] - секунды текущей минуты, 00-59
%[T] - время в формате ЧЧ-ММ
%[DATE] - дата в формате ДД_ММ_ГОД
%[E] - расширение файла в нижнем регистре (к примеру index.html имеет
       расширение html)
...
10. Интересные возможности
В этой части описываются некоторые расширенные возможности программы.
Вы наверняка заинтересуетесь в них после долгого использования.

10.1 FTP-поиск

Эта функция программы позволяет просто находить ближайший FTP сервер
содержайщий нужный вам файл. Для этого во время закачки файла, КОГДА
РАЗМЕР УЖЕ ОПРЕДЕЛИЛСЯ, нажмити правую кнопку мыши на закачке в очереди
закачек и выберите "FTP-поиск". В главном окне появится область отображающая
состояние поиска. После завершения поиска (такое задание будет помечено
галочкой) нажмите правой кнопкой мыши на задании поиска и выберите подходящий
сайт. Все сервера сортируются по времени доступа.

Различные настройки FTP-поиска присутсвуют в диалоге настроек в соответсвующем
пункте.

10.2 Планировщик

Планировщик позволяет планировать различные повторяющиеся действия для
автоматического исполненеия.

10.3 Пароли по умолчанию

Для любого сайта может быть заданы имя пользователя и пароль по умолчанию.
Заданная пара имя/пароль будут использоваться, если при добавлении новой
закачки с серевера  не были указаны пароль и имя пользователя. Доступ
к диалогу паролей осуществляется из главного меню:
в подменю "Настройки" выберите "Пароли".
