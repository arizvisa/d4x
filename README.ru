Руководство по использованию программы "Downloader for X" версии 1.12
...
описываемой версии.
    ...
    * возмодность ограничения пользователем максимальной скорости закачки,
    как для всех закачек, так и для отдельных
    * возмодность ограничивать количество подключений к заданному серверу
...
появится новая строка - это розличная информация о текущем положении дел
...
Вы можете настроить видимые кнопки через главное меню (5)
...
"ждать" ослабевания ограничений, например, останова других закачек (в
случае завершения, ошибки или действий пользователя). Отображаемый список
может иметь максимум 13 столбцов:
*солбец статуса - внем маленькой пиктограммой отображается текущее состояние
закачки,
*имя файла
...
Любой столбец можно отключить в "настройках программы" в разделе "Солбцы".
...
редактирование, перемещение, запуск, останов, введение ограничения, открытие
...
он пишется) нажатием правой кноки мышке на требуемой строке, выбрав
"Удалить строку". Чтобы скопировать строку из лога просто щелкните
два раза на строке и в появившемся диалоге выделите строку. Доступ к такоиу
...
екущая скорость в байтах в секунду
